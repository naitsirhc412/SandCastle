...
const { calculateMetrics, runWhatIf } = require('../../src/calculations');
  ...
  function tx(db, mode, fn) {
    return new Promise((resolve, reject) => {
      ...
      resolve(fn(t.objectStore(STORE), t.objectStore('meta'), t));
    });
  }
  ...
  async function getNextId(metaStore) {
    let id;
    try { id = await req2p(metaStore.get('nextId')); } catch(e) { id = null; }
    id = (id || 0) + 1;
    metaStore.put(id, 'nextId');
    return id;
  }

  // ── Parse helpers ──────────────────────────────────────────────────────────
  // sc-parser.js exposes window._SC.parseCSV; SheetJS exposes window.XLSX
  ...
  function parseExcelBuffer(buffer, csvType) {
    ...
    // Convert 2D array to CSV string and feed through the shared parser
  ...
  function parseAnyBuffer(buffer, filename, csvType) {
    ...
    // CSV / TSV — decode as UTF-8
    const text = new TextDecoder('utf-8').decode(new Uint8Array(buffer));
    return parseCSV(text, csvType);
  }

  // ── Metrics helpers ────────────────────────────────────────────────────────
  // mirrors the server-side calculations in src/db.js aggregatePeriods()
  ...
  async function dbGetAllPeriods() {
    ...
    const periods = await req2p((await openDB()).transaction(STORE, 'readonly').objectStore(STORE).getAll());
    db.close();
  ...
  async function getFilteredMetrics(viewBy, periodLabel) {
    ...
    if (periodLabel === labels[labels.length - 1] || !periodLabel) {
      // "Average" — last label is always the avg entry; for simplicity return latest specific
    }
    // Average across all = aggregate / count
  ...
  async function savePeriod(payload) {
    // Determine month keys from dated rows
    const allRows = [
      ...(payload.posRows     || []),
      ...(payload.payrollRows || []),
      ...(payload.expenseRows || []),
  ...
  window.api = {
    // ── File operations ────────────────────────────────────────────────────
    // pickCSV / pickFolder: not available in browser via dialog — the renderer
    // falls back to the visible file-picker <input> elements instead.
    // We return a cancellation so the renderer's existing fallback path is used.
    ...
    getMetrics: ({ viewBy, period }) =>
      getFilteredMetrics(viewBy, period).catch(() => ({})),

    getPeriodLabels: (viewBy) =>
      getPeriodLabels(viewBy).catch(() => []),

    deletePeriod: (id) =>
      dbDeletePeriod(id).then(() => ({ ok: true })).catch(e => ({ error: e.message })),

    patchPeriod: (payload) =>
      dbPatchPeriod(payload.id, payload.metrics).then(() => ({ ok: true })).catch(e => ({ error: e.message })),
    ...
    exportExcel: (payload) => {
      ...
      catch(e) { return Promise.resolve(exportCSVBrowser(payload)); } // fallback
    },
...
// Notify the app that the web API is ready (in case any code checks for it)
